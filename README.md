# `Sarasa Fixed CL Nerd` 字体

## 关于

`Sarasa Fixed CL Nerd` 字体是以 [Sarasa Fixed
CL](https://github.com/be5invis/Sarasa-Gothic)字体为基础，修改了[Nerd
fonts](https://github.com/ryanoasis/nerd-fonts)字体补丁程序，然后用该程序将`Nerd
fonts`合并入`Sarasa Fixed CL`, 再经过一些后处理，而最后形成的字体。该字体特别适合
**简体中文**用户在**终端**或者**代码编辑器**中使用。

上游版本：

- Sarasa Fixed CL：1.0.35
- Nerd Font: 3.3.0
- Font Patcher: 4.16.1
...
- `Sarasa Fixed CL` 是极少数做到中文和英文 2:1 严格对齐的字体，特别适合用来写代
...
- 对一些符号进行了纵向拉伸，不会出现`Powerline`条带中高低不一，无法上下对齐的情
  况。
- 原始`Sarasa Fixed CL`字体和`Sarasa Fixed CL Nerd`字体可以共存，不会产生冲突。
...
在你的主题配置文件中，使用 `Sarasa Fixed CL Nerd`。
...
wget -q $(curl 'https://api.github.com/repos/be5invis/Sarasa-Gothic/releases' | jq -r '.[0].assets | map(.browser_download_url) | map(select(test("SarasaFixedCL-TTF-[0-9.]+\\.7z"))) | .[0]')
7zr x Sarasa*.7z
mkdir sarasa
mv Sarasa*.ttf sarasa

# Download Font Patcher
wget -q https://github.com/ryanoasis/nerd-fonts/releases/download/v3.3.0/FontPatcher.zip
